...
| 数据库凭据 | user=`hermes`, password=`hermes`, dbname=`hermes` |
...
psql -U hermes -h 127.0.0.1 -d hermes -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER hermes WITH PASSWORD 'hermes';
...
```bash
# 连接数据库
psql -U hermes -h 127.0.0.1 -d hermes
...
# 执行 SQL 文件
psql -U hermes -h 127.0.0.1 -d hermes -f migration.sql
```
...
- [上游仓库](https://github.com/ca0fgh/Hermes)
