...
func TestWSResponseCreate_PassNormalizesFastAlias(t *testing.T) {
	...
	frame := []byte(`{"type":"response.create","model":"gpt-4","service_tier":"fast"}`)
	updated, blocked, err := svc.applyOpenAIFastPolicyToWSResponseCreate(context.Background(), account, "gpt-4", frame)
	require.NoError(t, err)
	require.Nil(t, blocked)
	require.Equal(t, "priority", gjson.GetBytes(updated, "service_tier").String(), "pass action must normalize fast alias before upstream forwarding")
}
