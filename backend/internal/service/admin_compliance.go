...
const (
	...
	AdminComplianceDocumentURLZH  = "https://github.com/ca0fgh/hermes-proxy/blob/main/docs/legal/admin-compliance.zh.md"
	AdminComplianceDocumentURLEN  = "https://github.com/ca0fgh/hermes-proxy/blob/main/docs/legal/admin-compliance.en.md"
	AdminComplianceAckPhraseZH    = "我已阅读、理解并同意 Hermes-Proxy 部署与运营合规承诺"
	AdminComplianceAckPhraseEN    = "I have read, understood, and agree to the Hermes-Proxy Deployment and Operation Compliance Commitment"
...
func (s *SettingService) GetAdminComplianceStatus(ctx context.Context, adminUserID int64) (*AdminComplianceStatus, error) {
	// 合规法务门禁已在本 fork 停用:始终返回「无需确认」,使 AdminComplianceGuard 成为
	// no-op 且前端确认弹框不再弹出(后续仍保留 ack 读写逻辑,便于将来需要时恢复)。
