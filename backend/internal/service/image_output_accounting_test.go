...
func TestOpenAIImageOutputCounter_NestedHermesProxy_StandardResponse(t *testing.T) {
	// Simulate what a nested hermes-proxy setup receives
	// When the upstream hermes-proxy returns a standard response, the downstream
	// hermes-proxy receives SSE events. This test verifies that no false positives
	// occur.
...
data: [DONE]`
	...
	count := countOpenAIImageOutputsFromSSEBody(sseBody)
	if count != 0 {
		t.Errorf("expected 0 images for nested hermes-proxy text-only response, got %d", count)
	}
}
