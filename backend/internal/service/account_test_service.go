...
import (
	...
	"github.com/ca0fgh/hermes-proxy/internal/pkg/openai_compat"
...
func (s *AccountTestService) testSoraAccountConnection(c *gin.Context, account *Account) error {
	...
	if resp.StatusCode != http.StatusOK {
		...
		upstreamCode, upstreamMessage := extractUpstreamErrorCodeAndMessage(body)
		switch {
	...
	subReq, err := http.NewRequestWithContext(ctx, http.MethodGet, soraBillingAPIURL, nil)
	if err == nil {
		...
		if subErr != nil {
			...
			if subResp.StatusCode == http.StatusOK {
				...
				if isCloudflareChallengeResponse(subResp.StatusCode, subResp.Header, subBody) {
					...
					upstreamCode, upstreamMessage := extractUpstreamErrorCodeAndMessage(subBody)
...
func (s *AccountTestService) testSora2Capabilities(
	...
	if inviteStatus != http.StatusOK {
		...
		upstreamCode, upstreamMessage := extractUpstreamErrorCodeAndMessage(inviteBody)
		if recorder != nil {
	...
	if remainingStatus != http.StatusOK {
		...
		upstreamCode, upstreamMessage := extractUpstreamErrorCodeAndMessage(remainingBody)
		if recorder != nil {
...
func extractCloudflareRayID(headers http.Header, body []byte) string {
	return utilhttputil.ExtractCloudflareRayID(headers, body)
}
...
func truncateSoraErrorBody(body []byte, max int) string {
	return utilhttputil.TruncateBody(body, max)
}
