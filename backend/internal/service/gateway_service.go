...
# Tone and style
 ...
 - Do not use a colon before tool calls. Your tool calls may not be shown directly in the output, so text like "Let me read the file:" followed by a read tool call should just be "Let me read the file." with a period.`
	...
	debugGatewayBodyEnv          = "HERMES_PROXY_DEBUG_GATEWAY_BODY"
...
type GatewayService struct {
	...
	debugGatewayBodyFile  atomic.Pointer[os.File] // non-nil when HERMES_PROXY_DEBUG_GATEWAY_BODY is set
...
func NewGatewayService(
	...
	svc.debugModelRouting.Store(parseDebugEnvBool(os.Getenv("HERMES_PROXY_DEBUG_MODEL_ROUTING")))
	svc.debugClaudeMimic.Store(parseDebugEnvBool(os.Getenv("HERMES_PROXY_DEBUG_CLAUDE_MIMIC")))
...
// hermesProxyUserID: 系统用户 ID，用于二维亲和调度
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, hermesProxyUserID int64) (*AccountSelectionResult, error) {
	// 调试日志：记录调度入口参数
...
// 启用方式（环境变量）：
//
//	HERMES_PROXY_DEBUG_GATEWAY_BODY=1                          # 写入 gateway_debug.log
//	HERMES_PROXY_DEBUG_GATEWAY_BODY=/tmp/gateway_debug.log     # 写入指定路径
//
