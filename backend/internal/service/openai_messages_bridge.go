...
func isOpenAICompatMessagesBridgeBody(body []byte) bool {
	...
	if bytes.Contains(body, []byte(openAICompatClaudeCodeTodoGuardDetect)) {
		return true
	}
...
func isOpenAICompatMessagesBridgeRequestBody(reqBody map[string]any) bool {
	...
	if input, ok := reqBody["input"].([]any); ok && inputContainsText(input, openAICompatClaudeCodeTodoGuardDetect) {
		return true
	}
