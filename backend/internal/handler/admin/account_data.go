...
const (
	dataType    = "hermes-proxy-data"
	dataVersion = 1
	dataPageCap = 1000
)

// acceptedLegacyDataTypes 列出导入时仍兼容接受的历史导出类型标识（sub2api 时期）。
// 导出端不再写出 type 字段（omitempty），此处仅用于让旧数据包继续可导入，零破坏。
var acceptedLegacyDataTypes = []string{"sub2api-data", "sub2api-bundle"}
...
func isAcceptedLegacyDataType(t string) bool {
	for _, legacy := range acceptedLegacyDataTypes {
		if t == legacy {
			return true
		}
	}
...
func validateDataHeader(payload DataPayload) error {
	if payload.Type != "" && payload.Type != dataType && !isAcceptedLegacyDataType(payload.Type) {
