#!/bin/bash
#
# hermes-proxy Installation Script
# hermes-proxy 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/ca0fgh/hermes-proxy/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="ca0fgh/hermes-proxy"
INSTALL_DIR="/opt/hermes-proxy"
SERVICE_NAME="hermes-proxy"
SERVICE_USER="hermes-proxy"
CONFIG_DIR="/etc/hermes-proxy"
...
declare -A MSG_ZH=(
    ...
    # Installation
    ["install_title"]="hermes-proxy 安装脚本"
    ...
    # Completion
    ["install_complete"]="hermes-proxy 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 hermes-proxy 服务："
    ...
    ["not_installed"]="hermes-proxy 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 hermes-proxy。"
    ...
    ["uninstall_complete"]="hermes-proxy 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 hermes-proxy"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 hermes-proxy"
    ...
    ["server_config_desc"]="配置 hermes-proxy 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="hermes-proxy Installation Script"
    ...
    # Completion
    ["install_complete"]="hermes-proxy installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start hermes-proxy service:"
    ...
    # Upgrade
    ["upgrading"]="Upgrading hermes-proxy..."
    ...
    ["not_installed"]="hermes-proxy is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove hermes-proxy from your system."
    ...
    ["uninstall_complete"]="hermes-proxy has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install hermes-proxy"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove hermes-proxy"
    ...
    ["server_config_desc"]="Configure hermes-proxy server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/hermes-proxy" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/hermes-proxy" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="hermes-proxy_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/hermes-proxy" "$INSTALL_DIR/hermes-proxy"
    chmod +x "$INSTALL_DIR/hermes-proxy"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/hermes-proxy"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/hermes-proxy.service << EOF
[Unit]
Description=hermes-proxy - AI API Gateway Platform
Documentation=https://github.com/ca0fgh/hermes-proxy
...
User=hermes-proxy
Group=hermes-proxy
WorkingDirectory=/opt/hermes-proxy
ExecStart=/opt/hermes-proxy/hermes-proxy
Restart=always
RestartSec=5
...
ProtectHome=true
PrivateTmp=true
ReadWritePaths=/opt/hermes-proxy
...
start_service() {
    ...
    if systemctl start hermes-proxy; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u hermes-proxy -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable hermes-proxy 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status hermes-proxy"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u hermes-proxy -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart hermes-proxy"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop hermes-proxy"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if hermes-proxy is installed
    if [ ! -f "$INSTALL_DIR/hermes-proxy" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/hermes-proxy" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet hermes-proxy; then
        print_info "$(msg 'stopping_service')"
        systemctl stop hermes-proxy
    fi

    # Backup current binary
    cp "$INSTALL_DIR/hermes-proxy" "$INSTALL_DIR/hermes-proxy.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/hermes-proxy.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/hermes-proxy"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start hermes-proxy

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: hermes-proxy must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/hermes-proxy" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="hermes-proxy.backup.${current_version}"
        else
            backup_name="hermes-proxy.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/hermes-proxy" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop hermes-proxy 2>/dev/null || true
    systemctl disable hermes-proxy 2>/dev/null || true
